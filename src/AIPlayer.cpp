...
void AIPlayer::think(color & c_piece, int & id_piece, int & dice) const{
    ...
    */
    ...
    switch(id) {
        case 0:
            podaAlphaBeta(actual,0,actual->getCurrentPlayerId(),c_piece,id_piece,dice,menosinf,masinf, ValoracionTest);
            break;
        case 1:
            podaAlphaBeta(actual,0,actual->getCurrentPlayerId(),c_piece,id_piece,dice,menosinf,masinf, Heuristica4);
            break;
        case 2:
        ...
        case 4:
            ...
            break;
        case 5:
            cout << "Valor MiniMax" << podaAlphaBeta(actual,0,actual->getCurrentPlayerId(),c_piece,id_piece,dice,menosinf,masinf, ValoracionTest);
            break;
        case 6:
            podaAlphaBeta(actual,0,jugador,c_piece,id_piece,dice,menosinf,masinf, Heuristica2);
            break;
        case 7:
            podaAlphaBeta(actual,0,actual->getCurrentPlayerId(),c_piece,id_piece,dice,menosinf,masinf, Heuristica4);
            break;
        case 8:
            podaAlphaBeta(actual,0,jugador,c_piece,id_piece,dice,menosinf,masinf, Heuristica3);
            break;
    }
}
...
double AIPlayer::evaluateItems(const Parchis& state, const int player){
    double item_score = 0;
...
double AIPlayer::Heuristica3(const Parchis &state, int player) {
    double color_score = 0, total_score = 0;
    const int max_distance = (65 + 8 + 1);
    
    const double score_goal = 70;
    const double score_corridor = 50;
    const double score_eat = 20;
    const double score_barrier = 15;
    const double score_safe = 15;
    const double score_out = 15;
    const double score_object = 2.5;
    ...
    for (int j = 0; j < 2; j++) {
        ...
        for (color c : colors) {
            for (int i = 0; i < num_pieces; i++) {
                double progress = (double)(max_distance - state.distanceToGoal(c, i)) + 50;
                bool is_safe = state.isSafePiece(c, i);
                bool is_corridor = state.getBoard().getPiece(c, i).get_box().type == final_queue;
                bool is_goal = state.getBoard().getPiece(c, i).get_box().type == goal;
                bool is_home = state.getBoard().getPiece(c,i).get_box().type == home;

                color_score += progress * 2;
                ...
                if (state.distanceToGoal(c, i) > max_distance-1)
                    color_score += 15;
